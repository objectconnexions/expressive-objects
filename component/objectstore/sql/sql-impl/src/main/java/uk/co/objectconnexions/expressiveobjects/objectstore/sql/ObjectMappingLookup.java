...
public class ObjectMappingLookup implements DebuggableWithTitle {
    ...
    public ObjectMapping getMapping(final ObjectSpecification spec, final DatabaseConnector connection) {
        ...
        if (mapping == null) {
            throw new ExpressiveObjectsException("No mapper for " + spec + " (no default mapper)");
        }
        return mapping;
    }
    ...
    private void add(final String className, final ObjectMapping mapper) {
        final ObjectSpecification spec = ExpressiveObjectsContext.getSpecificationLoader().loadSpecification(className);
    ...
    public void init() {
        ...
        final ExpressiveObjectsConfiguration subset = ExpressiveObjectsContext.getConfiguration().createSubset(prefix);
