...
package uk.co.objectconnexions.expressiveobjects.objectstore.xml.internal.utils;
...
public class CopyXmlObjectStore {
    ...
    private static void copyAllFiles(final String testDirectory, final String workingDirectory) {
        ...
        if (to.isFile()) {
            throw new ExpressiveObjectsException("To directory is actually a file " + to.getAbsolutePath());
        }
    ...
    private static void copyFile(final File from, final File to) {
        ...
        try {
            ...
            throw new ExpressiveObjectsException("Error copying file " + from.getAbsolutePath() + " to " + to.getAbsolutePath(), e);
        } finally {
            if (bis != null) {
                try {
                    bis.close();
