...
import uk.co.objectconnexions.expressiveobjects.core.commons.config.JmxBeanServer;
...
public class FileAuthorizor extends AuthorizorAbstract implements FileAuthorizorMBean {
    ...
    public FileAuthorizor(final ExpressiveObjectsConfiguration configuration) {
        super(configuration);
    ...
    private void findResources() {
        ...
        if (whiteListInputResource == null) {
            throw new ExpressiveObjectsException("Cannot read whitelist authorization file: " + whiteListResourceName);
        }
        ...
        if (blackListResourceName.length() > 0) {
            ...
            if (blackListInputResource == null) {
                throw new ExpressiveObjectsException("Blacklist authorization file exists, but it cannot be read: "
                    + blackListResourceName);
            }
        } else {
    ...
    private void tokenizeLine(final Map<String, List<String>> map, final String line) {
        ...
        if (pos == -1) {
            ...
            if (tokens.countTokens() != 2) {
                throw new ExpressiveObjectsConfigurationException("Invalid line: " + line);
            }
