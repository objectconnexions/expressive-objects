...
import uk.co.objectconnexions.expressiveobjects.viewer.dnd.view.InteractionSpy;
...
import uk.co.objectconnexions.expressiveobjects.viewer.dnd.view.UndoStack;
...
import uk.co.objectconnexions.expressiveobjects.viewer.dnd.view.base.NullView;
...
import uk.co.objectconnexions.expressiveobjects.viewer.dnd.view.debug.LoggingOptions;
import uk.co.objectconnexions.expressiveobjects.viewer.dnd.view.menu.PopupMenuContainer;
import uk.co.objectconnexions.expressiveobjects.viewer.dnd.view.message.MessageContent;
import uk.co.objectconnexions.expressiveobjects.viewer.dnd.viewer.ApplicationOptions;
...
public class XViewer implements Viewer {
    ...
    public XViewer() {
        doubleBuffering = ExpressiveObjectsContext.getConfiguration().getBoolean(Properties.PROPERTY_BASE + "double-buffer", true);
        showExplorationMenuByDefault = ExpressiveObjectsContext.getConfiguration().getBoolean(Properties.PROPERTY_BASE + "exploration.show", true);
    ...
    public void init() {
        ...
        if (ExpressiveObjectsContext.getConfiguration().getBoolean(Properties.PROPERTY_BASE + "show-mouse-spy", false)) {
            spy.open();
        }
    ...
    private Rectangle layoutViews() {
        if (!Thread.currentThread().getName().startsWith("AWT-EventQueue") && !isDotNet()) {
            ...
            // (Bug 1)
            throw new ExpressiveObjectsException("Drawing with wrong thread: " + Thread.currentThread());
        }
    ...
    public void saveOpenObjects() {
        ...
        ExpressiveObjectsContext.getUserProfileLoader().saveSession(objects);
    }
