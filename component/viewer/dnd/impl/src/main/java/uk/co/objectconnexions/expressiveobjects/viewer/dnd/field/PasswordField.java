...
public class PasswordField extends TextField {
    ...
    public PasswordField(final Content content, final ViewSpecification design) {
        ...
        final String echoCharacterSetting = ExpressiveObjectsContext.getConfiguration().getString(Properties.PROPERTY_BASE + "echo");
    ...
    protected void drawText(final Canvas canvas, final Color textColor, final int width) {
        ...
        if (lines.length > 1) {
            throw new ExpressiveObjectsException("Password field should contain a string that contains no line breaks; contains " + lines.length);
        }
