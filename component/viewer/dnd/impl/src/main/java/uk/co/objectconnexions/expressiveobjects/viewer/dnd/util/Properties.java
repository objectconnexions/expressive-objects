...
package uk.co.objectconnexions.expressiveobjects.viewer.dnd.util;
...
public class Properties {
    ...
    public static Size getSize(final String name, final Size defaultSize) {
        ...
        if (initialSize == null) {
            initialSize = ExpressiveObjectsContext.getConfiguration().getString(name);
        }
        if (initialSize != null) {
            ...
            if (st.countTokens() == 2) {
                ...
                throw new ExpressiveObjectsConfigurationException("Size not specified correctly in " + name + ": " + initialSize);
            }
        }
        return defaultSize;
    }
    ...
    public static void saveSizeOption(final String name, final Size size) {
        ...
        ExpressiveObjectsContext.getUserProfile().getOptions().addOption(name, value);
    }
    ...
    public static Location getLocation(final String name, final Location defaultLocation) {
        ...
        if (initialLocation == null) {
            initialLocation = ExpressiveObjectsContext.getConfiguration().getString(name);
        }
        if (initialLocation != null) {
            ...
            if (st.countTokens() == 2) {
                ...
                throw new ExpressiveObjectsConfigurationException("Location not specified correctly in " + name + ": " + initialLocation);
            }
        }
    ...
    private static String optionFor(final String name) {
        return ExpressiveObjectsContext.inSession() ? ExpressiveObjectsContext.getUserProfile().getOptions().getString(name) : null;
    }
    ...
    public static String getString(final String name) {
        ...
        if (value == null) {
            value = ExpressiveObjectsContext.getConfiguration().getString(PROPERTY_BASE + name);
        }
        return value;
    }

    public static void setStringOption(final String name, final String value) {
        ExpressiveObjectsContext.getUserProfile().getOptions().addOption(PROPERTY_BASE + name, value);
    }

    public static Options getOptions(final String name) {
        return ExpressiveObjectsContext.getUserProfile().getOptions().getOptions(name);
    }
