...
public class ApplicationWorkspaceBuilder extends AbstractViewBuilder {
    ...
    public void build(final View view1, final Axes axes) {
        ...
        for (final Object object : perspective.getObjects()) {
            final ObjectAdapter adapter = ExpressiveObjectsContext.getPersistenceSession().getAdapterManager().adapterFor(object);
        ...
        for (final Object service : perspective.getServices()) {
            final ObjectAdapter adapter = ExpressiveObjectsContext.getPersistenceSession().getAdapterManager().adapterFor(service);
            if (isHidden(adapter)) {
                continue;
            }
