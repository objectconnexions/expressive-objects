...
import uk.co.objectconnexions.expressiveobjects.viewer.scimpi.dispatcher.action.ActionAction;
...
import uk.co.objectconnexions.expressiveobjects.viewer.scimpi.dispatcher.logon.LogonAction;
import uk.co.objectconnexions.expressiveobjects.viewer.scimpi.dispatcher.logon.LogoutAction;
import uk.co.objectconnexions.expressiveobjects.viewer.scimpi.dispatcher.processor.Encoder;
import uk.co.objectconnexions.expressiveobjects.viewer.scimpi.dispatcher.processor.HtmlFileParser;
import uk.co.objectconnexions.expressiveobjects.viewer.scimpi.dispatcher.processor.ProcessorLookup;
import uk.co.objectconnexions.expressiveobjects.viewer.scimpi.dispatcher.processor.Request;
import uk.co.objectconnexions.expressiveobjects.viewer.scimpi.dispatcher.processor.SimpleEncoder;
...
public class Dispatcher {
    ...
    public void process(final RequestContext context, final String servletPath) {
        ...
        ExpressiveObjectsContext.getPersistenceSession().getTransactionManager().startTransaction();
        ...
        try {
            ...
            ErrorCollator error = new ErrorCollator();
            final PersistenceSession checkSession = ExpressiveObjectsContext.getPersistenceSession();
            final ExpressiveObjectsTransactionManager transactionManager = checkSession.getTransactionManager();
    ...
    private void redirectToLoginPage(final RequestContext context) {
        ExpressiveObjectsContext.getMessageBroker().addWarning(
    ...
    protected void processTheView(final RequestContext context) throws IOException {
        ExpressiveObjectsTransactionManager transactionManager = ExpressiveObjectsContext.getPersistenceSession().getTransactionManager();
        ...
        // processing above.
        transactionManager = ExpressiveObjectsContext.getPersistenceSession().getTransactionManager();
        if (transactionManager.getTransaction().getState().canCommit()) {
            ExpressiveObjectsContext.getPersistenceSession().getTransactionManager().endTransaction();
        }
    ...
    public void init(final String dir, final DebugUsers debugUsers) {
        ...
        processors.init();
        processors.addElementProcessor(new uk.co.objectconnexions.expressiveobjects.viewer.scimpi.dispatcher.view.debug.Debug(this));
        
        showUnshownMessages = ExpressiveObjectsContext.getConfiguration().getBoolean(SHOW_UNSHOWN_MESSAGES, true);
    }
