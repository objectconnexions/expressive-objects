...
package uk.co.objectconnexions.expressiveobjects.viewer.scimpi.dispatcher.debug;
...
public class DebugUsers {
    ...
    public void initialize() {
        ...
        final String debugUserEntry = ExpressiveObjectsContext.getConfiguration().getString(ConfigurationConstants.ROOT + "scimpi.debug.users", "");
        final String[] users = debugUserEntry.split("\\|");
        for (final String name : users) {
            debugUsers.add(name.trim());
        }

        final String debugModeEntry = ExpressiveObjectsContext.getConfiguration().getString(ConfigurationConstants.ROOT + "scimpi.debug.mode");
