...
import uk.co.objectconnexions.expressiveobjects.core.progmodel.facets.object.choices.enums.EnumFacet;
...
import uk.co.objectconnexions.expressiveobjects.viewer.scimpi.dispatcher.view.form.HtmlFormBuilder;
...
public class EditObject extends AbstractElementProcessor {
    ...
    public void process(final Request request) {
        ...
        final FormFieldBlock containedBlock = new FormFieldBlock() {
            ...
            public boolean isVisible(final String name) {
                ...
                final boolean isVisible = fld.isVisible(ExpressiveObjectsContext.getAuthenticationSession(), object, where).isAllowed();
                final boolean isUseable = fld.isUsable(ExpressiveObjectsContext.getAuthenticationSession(), object, where).isAllowed();
    ...
    private void copyEntryState(final RequestContext context, final ObjectAdapter object, final InputField[] formFields, final FormState entryState) {
        for (final InputField formField : formFields) {
            final String fieldId = formField.getName();
            final ObjectAssociation field = object.getSpecification().getAssociation(fieldId);
            if (field.isVisible(ExpressiveObjectsContext.getAuthenticationSession(), object, where).isAllowed() && formField.isEditable()) {
