...
package uk.co.objectconnexions.expressiveobjects.core.metamodel.services.container;
...
import uk.co.objectconnexions.expressiveobjects.applib.RepositoryException;
...
import uk.co.objectconnexions.expressiveobjects.core.metamodel.adapter.LocalizationProviderAware;
...
import uk.co.objectconnexions.expressiveobjects.core.metamodel.adapter.QuerySubmitterAware;
...
public class DomainObjectContainerDefault implements DomainObjectContainer, QuerySubmitterAware, ObjectDirtierAware, DomainObjectServicesAware, ObjectPersistorAware, SpecificationLoaderAware, AuthenticationSessionProviderAware, AdapterManagerAware, LocalizationProviderAware {
    ...
    public <T> T newAggregatedInstance(final Object parent, final Class<T> ofClass) {
        final ObjectSpecification spec = getSpecificationLookup().loadSpecification(ofClass);
        if (!spec.isParented()) {
            throw new ExpressiveObjectsException("Type must be annotated as @Aggregated: " + ofClass);
        }
        final ObjectAdapter adapter = doCreateAggregatedInstance(spec, parent);
        if (adapter.getOid() instanceof AggregatedOid) {
            return (T) adapter.getObject();
        } else {
            throw new ExpressiveObjectsException("Object instantiated but was not given a AggregatedOid (does the configured object store support aggregates?): " + ofClass);
        }
    }
