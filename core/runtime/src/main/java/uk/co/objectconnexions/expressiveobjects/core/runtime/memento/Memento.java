...
import uk.co.objectconnexions.expressiveobjects.core.commons.encoding.DataInputStreamExtended;
...
import uk.co.objectconnexions.expressiveobjects.core.metamodel.adapter.oid.ParentedOid;
...
//            }
    ...
    private void updateObject(final ObjectAdapter adapter, final Data data) {
        ...
        if (!(data instanceof ObjectData)) {
            throw new ExpressiveObjectsException("Expected an ObjectData but got " + data.getClass());
        }
    ...
    private void updateFieldsAndResolveState(final ObjectAdapter objectAdapter, final Data data) {
        ...
        if (!dataIsTransient) {
            ...
            if (od.containsField()) {
                throw new ExpressiveObjectsException("Resolve state (for " + objectAdapter + ") inconsistent with fact that data exists for fields");
            }
        }
    }
