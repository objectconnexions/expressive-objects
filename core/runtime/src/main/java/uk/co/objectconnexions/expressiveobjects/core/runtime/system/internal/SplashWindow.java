...
import uk.co.objectconnexions.expressiveobjects.core.runtime.imageloader.TemplateImage;
...
public class SplashWindow extends Window implements Runnable {
    ...
    private static final String LOGO_TEXT = "Expressive Objects";
    ...
    public SplashWindow(final TemplateImageLoader loader) {
        ...
        final String imageName = AboutExpressiveObjects.getImageName();
        final TemplateImage templateImage = loader.getTemplateImage(imageName);
        if (templateImage == null) {
            throw new ExpressiveObjectsException("Failed to find splash image " + imageName);
        }
    ...
    private Dimension textBounds() {
        ...
        // framework details
        width = titleMetrics.stringWidth(AboutExpressiveObjects.getFrameworkName());
        height += titleLineHeight;
        width = Math.max(width, textMetrics.stringWidth(AboutExpressiveObjects.getFrameworkCopyrightNotice()));
    ...
    public void paint(final Graphics g) {
        ...
        g.drawString(AboutExpressiveObjects.getFrameworkCopyrightNotice(), left, baseline);
        baseline -= textLineHeight;
        g.setFont(titleFont);
        g.drawString(AboutExpressiveObjects.getFrameworkName(), left, baseline);
        baseline -= titleLineHeight;

        // application details - from bottom to top
        g.setFont(textFont);
        final String applicationVersion = AboutExpressiveObjects.getApplicationVersion();
        if (applicationVersion != null) {
            g.drawString(applicationVersion, left, baseline);
            baseline -= textLineHeight;
        }
        final String applicationCopyrightNotice = AboutExpressiveObjects.getApplicationCopyrightNotice();
        if (applicationCopyrightNotice != null) {
            g.drawString(applicationCopyrightNotice, left, baseline);
            baseline -= textLineHeight;
        }
        final String applicationName = AboutExpressiveObjects.getApplicationName();
