...
public class UserProfileLoaderDefault implements UserProfileLoader, DebuggableWithTitle {
    ...
    private static final String DEFAULT_PERSPECTIVE_NAME = "Expressive Objects";
    ...
    private UserProfile createDefaultProfile(final String userName) {
        ...
        profile.newPerspective(DEFAULT_PERSPECTIVE_NAME + (ExpressiveObjectsContext.getDeploymentType().isExploring() ? EXPLORATION : ""));

        final List<Object> services = getServices();
        if (services.size() == 0 && mode == Mode.STRICT) {
            throw new ExpressiveObjectsException("No known services");
        }
