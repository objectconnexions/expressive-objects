...
import uk.co.objectconnexions.expressiveobjects.core.webapp.auth.AuthenticationSessionStrategy;
...
public class ExpressiveObjectsSessionFilter implements Filter {
    ...
    public enum WhenNoSession {
        UNAUTHORIZED("unauthorized") {
            @Override
            public void handle(final ExpressiveObjectsSessionFilter filter, final HttpServletRequest httpRequest, final HttpServletResponse httpResponse, final FilterChain chain) throws IOException, ServletException {
        ...
        BASIC_AUTH_CHALLENGE("basicAuthChallenge") {
            @Override
            public void handle(final ExpressiveObjectsSessionFilter filter, final HttpServletRequest httpRequest, final HttpServletResponse httpResponse, final FilterChain chain) throws IOException, ServletException {
                httpResponse.setHeader("WWW-Authenticate", "Basic realm=\"Expressive Objects\"");
                httpResponse.sendError(401);
            }
        },
        /**
        ...
        public abstract void handle(ExpressiveObjectsSessionFilter filter, HttpServletRequest httpRequest, HttpServletResponse httpResponse, FilterChain chain) throws IOException, ServletException;
    }

    /**
     * Used as a flag on {@link HttpServletRequest} so that if there are two
     * {@link ExpressiveObjectsSessionFilter}s in a request pipeline, then the second can
     * determine what processing has already been done (and is usually then just
     * a no-op).
     */
    private static final String SESSION_STATE_KEY = ExpressiveObjectsSessionFilter.SessionState.class.getName();
    ...
    public enum SessionState {
        ...
        UNDEFINED {
            @Override
            public void handle(final ExpressiveObjectsSessionFilter filter, final ServletRequest request, final ServletResponse response, final FilterChain chain) throws IOException, ServletException {
            ...
            private boolean requestIsIgnoreExtension(final ExpressiveObjectsSessionFilter filter, final HttpServletRequest httpRequest) {
